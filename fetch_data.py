...
def fetch_kerala_csv() -> list:
    ...
    for page in range(MAX_ATTEMPTS * 10):
        ...
        if page_records is None:
            raise RuntimeError(
                f"Gave up fetching page {page + 1} after {MAX_ATTEMPTS} attempts: {last_error}"
            )
        ...
        print(
            f"[fetch] page {page + 1}: fetched {len(page_records)} records "
